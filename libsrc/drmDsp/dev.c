...
int is_supported_format(struct sp_plane* plane, uint32_t format) {
  ...
  for (i = 0; i < plane->plane->count_formats; i++) {
	printf("====================> plane_id=%d crtc_id=%d fb_id = %d format=%d, %d\n",
			plane->plane->plane_id, plane->plane->crtc_id, plane->plane->fb_id,
			plane->plane->formats[i], format);
    if (plane->plane->formats[i] == format)
    //if (plane->plane->formats[i] == format && plane->plane->crtc_id == 83)
    //if (plane->plane->crtc_id == 83)
      return 1;
  }
  return 0;
}
...
#if 1
  for (i = 0; i < plane->plane->count_formats; i++) {
	printf("yk debug support formats:%d  ", plane->plane->formats[i]);
	if(plane->plane->formats[i] == DRM_FORMAT_XRGB8888)
		printf("DRM_FORMAT_XRGB8888\n");
		...
		printf("DRM_FORMAT_BGR888\n");
	else if(plane->plane->formats[i] == DRM_FORMAT_RGB565)
		printf("DRM_FORMAT_RGB565\n");
	else if(plane->plane->formats[i] == DRM_FORMAT_BGR565)
		printf("DRM_FORMAT_BGR565\n");
	else if(plane->plane->formats[i] == DRM_FORMAT_ARGB1555)
...
#if 0
	if(plane->plane->formats[i]  == DRM_FORMAT_BGR565)
	{
...
struct sp_dev* create_sp_dev(void) {
  ...
  dev->num_connectors = r->count_connectors;
	printf("************yk debug connector num:%d\n", dev->num_connectors);
  ...
  for (i = 0; i < dev->num_planes; i++) {
	...
	printf("=========> yk debug  support format = %d\n", plane->format);
