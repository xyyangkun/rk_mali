...
static int map_sp_bo(struct sp_bo* bo) {
  ...
  ret = drmPrimeHandleToFD(bo->dev->fd, bo->handle, 0, &bo->fd);
  if(ret < 0) {
	  printf("failed to get fd ret=%d\n", -errno);
	  return -errno;
  }
  printf("***********yk debug map_addr:%p, fd=%d\n", bo->map_addr, bo->fd);
  return 0;
}
...
struct sp_bo* create_sp_bo(struct sp_dev* dev, uint32_t width, uint32_t height,
  ...
  printf("yk debug drmIoctl width:%d height:%d\n", cd.width, cd.height);
  printf("yk debug size = %d patch=%d\n", cd.size, cd.pitch);
