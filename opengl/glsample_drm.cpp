...
int main() {
	...
	init_display_mem(w, h, out_type, 1);
    ...
    gbm = gbm_create_device(display_get_fd());
    if (gbm == NULL) {
	...
	*/
    ...
    // 4. Create Surface
    gbmSurface = gbm_surface_create(gbm, w, h,
                            GBM_FORMAT_BGRA8888,
	...
	if(bo == NULL) {
		printf("ERROR to get gbm buffer\n");
		return -1;
	}
	...
	printf("bo user data:%p\n", gbm_bo_get_user_data(bo));

	bo_t bo_info;
	bo_info.handle = handle;
	int bo_fd = 0; 
	c_RkRgaGetBufferFd(&bo_info, &bo_fd);
	printf("bo_fd=%d\n", bo_fd);
#if 1
	int _fd; 
	void *_ptr;
	...
	while(1) {
		...
		//memcpy(_ptr, gbm_bo_get_user_data(bo), 1920*1080);
		memset(_ptr, 0, 1920*1080);
		usleep(20*1000);
	}
#endif
...
#if 0
	printf("============> v:%d h:%d, stride:%d \n", kms.mode.hdisplay, kms.mode.vdisplay, stride);
