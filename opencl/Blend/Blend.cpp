...
int main(int argc, char** argv)
{
    ...
    program = CreateProgram(context, device, "Blend.cl");
    if (program == NULL)
    {
        Cleanup(context, commandQueue, program, kernel, memObjects);
        return 1;
    ...
    kernel = clCreateKernel(program, "blend_y", NULL);
    if (kernel == NULL)
    {
	...
	printf("memObjects 0:%p\n", &memObjects[0]);
    // Set the kernel arguments (w ,h ,....)
    errNum  = clSetKernelArg(kernel, 0, sizeof(int), &w1);
    errNum |= clSetKernelArg(kernel, 1, sizeof(int), &h1);
    errNum |= clSetKernelArg(kernel, 2, sizeof(int), &w2);
    errNum |= clSetKernelArg(kernel, 3, sizeof(int), &h2);
    errNum |= clSetKernelArg(kernel, 4, sizeof(int), &x);
    errNum |= clSetKernelArg(kernel, 5, sizeof(int), &y);
    errNum |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &memObjects[0]);
    errNum |= clSetKernelArg(kernel, 7, sizeof(cl_mem), &memObjects[1]);
    if (errNum != CL_SUCCESS)
    {
...
/////////////////////////////////////////////////////////////////////////////
	...
	// 合成UV
    // Create OpenCL kernel
    kernel = clCreateKernel(program, "blend_uv", NULL);
    if (kernel == NULL)
    {
        std::cerr << "Failed to create kernel" << std::endl;
        Cleanup(context, commandQueue, program, kernel, memObjects);
